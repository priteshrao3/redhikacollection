...
export function OrderSummary({
  ...
  const shipping = subtotal >= settings.freeShippingThreshold || subtotal === 0 ? 0 : settings.standardShippingFee;
  const discount = subtotal >= settings.discountThreshold ? Math.round(subtotal * settings.discountRate) : 0;
  ...
  return (
    <div className="rounded-xl border border-neutral-200 p-5">
      ...
      <p className="mt-3 text-center text-xs text-neutral-400">{settings.paymentMethodsText}</p>
    </div>
  );
}
